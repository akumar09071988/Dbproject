...
public class TransactionMB 
{
  ...
  public String navigateToPayment()
  {
	  ...
	  FacesContext context = FacesContext.getCurrentInstance();
	  HttpSession session = (HttpSession) context.getExternalContext().getSession(true);
	  session.setAttribute("transactionObj", outer);
	  return "paymentClient";
  }
  
  public void cancelTransaction(ActionEvent ae)
  {
	  for(int i=0;i<transList.size();i++)
	  {
		  TransactionVO inner = transList.get(i);
		  ...
		  {
			  TransactionDao dao = new TransactionDao();
			  dao.cancelTransaction(inner.getTransactionID());
			  transList = dao.getTransaction(inner.getClientID());
		  }
	  }
  }
